  ...
  if (!valid) {
    return (
      <div className="flex min-h-[var(--app-h)] flex-col items-start justify-center bg-ink px-7 text-cream">
        <Logo h="h-10" />
        <h1 className="display mt-5 text-[32px] font-bold leading-tight">{t("this code's used up.")}</h1>
    ...
    <div className="flex min-h-[var(--app-h)] flex-col bg-ink px-6 text-cream">
      ...
      <main className="flex flex-1 flex-col justify-center pb-8">
        ...
        <motion.h1 initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="display mt-1.5 text-[34px] font-bold leading-[1.05]">
          {t("someone vouched for you. no application, no fee.")}
        </motion.h1>
        <p className="mt-3 text-[14px] leading-relaxed text-cream/70">{t("just tell us who you are so your pass has a name on it.")}</p>

        <div className="mt-7 space-y-4">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("first name")}
          ...
          <div className="flex items-center gap-2 border-b-2 border-cream/15 pb-2 focus-within:border-lime">
            ...
            <input value={ig} onChange={(e) => setIg(e.target.value.replace(/\s/g, ""))} placeholder={t("@instagram")}
          ...
          <div className="flex items-center gap-2 border-b-2 border-cream/15 pb-2 focus-within:border-lime">
            ...
            <input type="email" autoCapitalize="none" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t("email")}
      ...
      <footer className="pb-[max(1.25rem,env(safe-area-inset-bottom))]">
        ...
        <motion.button whileTap={{ scale: 0.97 }} onClick={redeem} disabled={!ready || busy}
          ...
          {busy ? t("letting you in…") : t("claim my pass")}
        </motion.button>
      </footer>
    </div>
  )
